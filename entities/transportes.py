...
"""Clase Pydantic para validación de transportes"""
class TransporteBase(BaseModel):
    tipo_vehiculo: str = Field(..., min_length=1, max_length=50, description="Tipo de vehículo")
    capacidad_carga: float = Field(..., gt=0, description="Capacidad de carga en kilogramos")
    ...
    marca: str = Field(..., min_length=1, max_length=50, description="Marca del vehículo")
    año: int = Field(..., ge=1900, le=2030, description="Año del vehículo")
    estado: str = Field(..., min_length=1, max_length=20, description="Estado del vehículo")
    
    @validator('tipo_vehiculo')
    def validar_tipo_vehiculo(cls, v):
        tipos_validos = ["camión", "moto", "furgoneta", "camioneta", "bicicleta"]
        if v.lower() not in tipos_validos:
            raise ValueError(f'El tipo de vehículo debe ser uno de: {", ".join(tipos_validos)}')
    ...
    def validar_capacidad_carga(cls, v):
        ...
        if v > 50000:  # 50 toneladas máximo
    ...
    def validar_placa(cls, v):
        ...
        # Patrón básico para placas (puede ajustarse según el país)
        if not re.match(r'^[A-Z0-9]{6,10}$', v.upper().replace('-', '').replace(' ', '')):
...
class TransporteCreate(TransporteBase):
    """Clase Pydantic para validación de creación de transportes"""
    pass

class TransporteUpdate(BaseModel):
    """Clase Pydantic para validación de actualización de transportes"""
...
class TransporteResponse(TransporteBase):
    """Esquema para respuesta de transporte"""
    id_transporte: UUID
...
class TransporteListResponse(BaseModel):
    """Esquema para lista de transportes"""
