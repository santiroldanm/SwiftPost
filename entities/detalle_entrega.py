...
class DetalleEntrega(Base):
    ...
    id_detalle = Column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    id_sede_remitente = Column(UUID(as_uuid=True), ForeignKey('sedes.id_sede'), nullable=False)
    id_sede_receptora = Column(UUID(as_uuid=True), ForeignKey('sedes.id_sede'), nullable=False)
    id_paquete = Column(UUID(as_uuid=True), ForeignKey('paquetes.id'), nullable=False)
    estado_envio = Column(String(20), default="Pendiente", nullable=False)
    id_cliente_remitente = Column(UUID(as_uuid=True), ForeignKey('clientes.id_cliente'), nullable=False)
    id_cliente_receptor = Column(UUID(as_uuid=True), ForeignKey('clientes.id_cliente'), nullable=False)
    ...
    sede_remitente = relationship("Sede", foreign_keys=[id_sede_remitente], back_populates="sede_remitente")
    sede_receptora = relationship("Sede", foreign_keys=[id_sede_receptora], back_populates="sede_receptora")
    paquete = relationship("Paquete", back_populates="detalles_entrega")
    cliente_remitente = relationship("Cliente", foreign_keys=[id_cliente_remitente], back_populates="cliente_remitente")
    cliente_receptor = relationship("Cliente", foreign_keys=[id_cliente_receptor], back_populates="cliente_receptor")
...
class DetalleEntregaBase(BaseModel):
    """Clase base para validación de detalles de entrega"""
    ...
    def validar_estado_envio(cls, v):
        if v not in ["Pendiente", "En transito", "Entregado"]:
            raise ValueError('El estado del envío debe ser "Pendiente", "En transito" o "Entregado"')
        return v
...
class DetalleEntregaCreate(DetalleEntregaBase):
    """Clase para validación de creación de detalles de entrega"""
    pass

class DetalleEntregaUpdate(BaseModel):
    """Clase para validación de actualización de detalles de entrega"""
    ...
    def validar_fecha_entrega(cls, v, values):
        if v and 'fecha_envio' in values and v < values['fecha_envio']:
            raise ValueError('La fecha de entrega no puede ser anterior a la fecha de envío')
        return v
...
class DetalleEntregaResponse(DetalleEntregaBase):
    """Esquema para respuesta de detalle de entrega"""
    id_detalle: UUID
...
class DetalleEntregaListResponse(BaseModel):
    """Esquema para lista de detalles de entrega"""
