...
def listar_empleados(db: Session) -> None:
    ...
    try:
        ...
        for empleado in empleados:
            # Obtener el nombre del tipo de documento usando la relación correcta
            nombre_tipo_doc = 'N/A'
            ...
            datos_empleados.append([
                ...
                empleado.documento,  # Cambiado de numero_documento a documento
                nombre_tipo_doc,
                empleado.correo,
                empleado.telefono,
        ...
        headers = ["ID", "Nombre Completo", "Documento", "Tipo Doc.", "Correo", "Teléfono", "Estado"]
        ...
        for empleado_data in datos_empleados:
            row_line = (f"{str(empleado_data[0]):<{col_id}} | "
...
def buscar_empleado(db: Session) -> None:
    ...
    try:
        termino = input("\nIngrese el número de documento o nombre del empleado: ").strip()
        ...
        for empleado in empleados:
            ...
            print(f"Nombre: {empleado.primer_nombre} {empleado.segundo_nombre or ''} {empleado.primer_apellido} {empleado.segundo_apellido or ''}")
            print(f"Documento: {empleado.tipo_documento.nombre if empleado.tipo_documento else 'N/A'} {empleado.numero_documento}")
...
def registrar_empleado(db: Session, id_administrador: str = None) -> None:
    ...
    try:
        ...
        # Mostrar tipos de documento disponibles con información de depuración
        ...
        # Confirmar datos
        print("\nResumen de datos:")
        print(f"Nombre: {primer_nombre} {segundo_nombre or ''} {primer_apellido} {segundo_apellido or ''}")
        ...
        empleado_data = {
            "id_empleado": id_usuario,  # ID único para el empleado
            ...
            "activo": True,
            "usuario_id": id_usuario  # Mismo ID para la relación con usuario
        }
        ...
        # Convertir el diccionario a un objeto EmpleadoCreate
        from entities.empleado import EmpleadoCreate
        from uuid import UUID
        
        # Usar el ID del administrador que está realizando la acción
        empleado_data['creado_por'] = str(id_administrador)
        # Usar el ID del usuario que se está creando
        empleado_data['usuario_id'] = str(id_usuario)
        
        try:
            # Convertir los UUID a cadenas de texto
            empleado = empleado_crud.crear(
                db=db,
                datos=empleado_data,
                usuario_id=str(id_usuario),         # ID del nuevo usuario/empleado como cadena
                creado_por=str(id_administrador)    # ID del administrador como cadena
            )
        ...
        # Preguntar si se desea crear un usuario para este empleado
        crear_usuario = input("\n¿Desea crear un usuario para este empleado? (s/n): ").strip().lower()
...
def editar_empleado(db: Session) -> None:
    ...
    try:
        ...
        if datos_actualizados:
            empleado_actualizado = empleado_crud.update(db, db_obj=empleado, obj_in=datos_actualizados)
...
def cambiar_estado_empleado(db: Session) -> None:
    ...
    try:
        id_empleado = input("\nIngrese el ID del empleado a activar/desactivar: ").strip()
        ...
        confirmar = input(f"\n¿Desea {'DESACTIVAR' if empleado.activo else 'ACTIVAR'} este empleado? (s/n): ").strip().lower()
