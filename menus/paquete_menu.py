...
def registrar_paquete(db: Session, id_creador: str) -> None:
    ...
    try:
        # Lógica para registrar un paquete
...
def listar_paquetes(db: Session) -> None:
    ...
    try:
        ...
        if not paquetes:
            ...
            print(f"{'ID':<{col_id}} | {'CLIENTE':<{col_cliente}} | {'CONTENIDO':<{col_contenido}} | {'ESTADO':<{col_estado}} | {'FECHA REGISTRO':<{col_fecha}}")
            print("-" * (col_id + col_cliente + col_contenido + col_estado + col_fecha + 12))  # +12 por los separadores " | "
            
            # Imprimir cada paquete
            for p in paquetes:
                # Usar getattr para acceder a los atributos de manera segura
                cliente = getattr(p, 'cliente', None)
                cliente_nombre = f"{getattr(cliente, 'primer_nombre', '')} {getattr(cliente, 'primer_apellido', '')}".strip() if cliente else "N/A"
                
                print(f"{getattr(p, 'id_paquete', 'N/A'):<{col_id}} | "
...
def rastrear_envio(db: Session, usuario_data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        input("\nPresione Enter para continuar...")
        return
    if paquete and paquete.id_cliente == cliente_crud.obtener_por_usuario(db, usuario_id=usuario_data['id_usuario']).id_cliente:
...
def historial_envios(db: Session, usuario_data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if not paquetes or total == 0:
            ...
            print("-" * (col_id + col_contenido + col_estado + col_fecha + 9))  # +9 por los separadores " | "
            ...
            for p in paquetes:
                ...
                print(f"{id_paquete:<{col_id}} | {contenido:<{col_contenido}} | {estado:<{col_estado}} | {fecha:<{col_fecha}}")
    except Exception as e:
