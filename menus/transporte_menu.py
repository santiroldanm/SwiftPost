...
def listar_transportes(db: Session) -> None:
    ...
    try:
        ...
        for t in transportes:
            ...
            row_line = (f"{str(t.id_transporte):<{col_id}} | "
...
def agregar_transporte(db: Session, id_administrador: str) -> None:
    ...
    try:
        ...
        # Resto de los datos del transporte
        marca = input("\nMarca: ").strip()
        modelo = input("Modelo: ").strip()
        placa = input("Placa: ").strip().upper()
        
        # Validar año (mínimo 2002 según el modelo)
        while True:
            año_str = input("Año: ").strip()
            try:
                año = int(año_str)
                if 2002 <= año <= datetime.now().year + 1:  # Mínimo 2002, máximo año actual + 1
                    break
        ...
        # Crear el objeto TransporteCreate con los datos validados
        ...
        transporte_data = TransporteCreate(
            ...
            estado='disponible',
            activo=True,
            id_sede=str(sede_seleccionada.id_sede)  # Asegurarse de convertir a string
        )
        
        # Llamar al método crear con el objeto validado
...
def cambiar_estado_transporte(db: Session) -> None:
    ...
    confirmar = input(f"¿Está seguro de {'activar' if nuevo_estado else 'desactivar'} el vehículo {placa}? (s/n): ").lower()
    if confirmar == 's':
