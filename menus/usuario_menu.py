...
def listar_usuarios(db: Session) -> None:
    ...
    try:
        usuarios = (
            db.query(Usuario, Rol.nombre_rol.label("rol_nombre"))
        ...
        column_widths = [36, 20, 20, 10, 16, 16]
        ...
        for usuario, rol_nombre in usuarios:
            ...
            fecha_creacion = (
                usuario.fecha_creacion.strftime("%Y-%m-%d %H:%M")
...
def buscar_usuario(db: Session) -> None:
    ...
    try:
        ...
        if tipo_cuenta == "Cliente" and hasattr(usuario, "cliente") and usuario.cliente:
            ...
            print(
                f"Apellidos: {getattr(usuario.cliente, 'apellidos', 'No especificado')}"
            )
            print(
                f"Teléfono: {getattr(usuario.cliente, 'telefono', 'No especificado')}"
            )

        elif (
            tipo_cuenta == "Empleado"
            and hasattr(usuario, "empleado")
            ...
            print(
                f"Apellidos: {getattr(usuario.empleado, 'apellidos', 'No especificado')}"
            )
    ...
    try:
        ...
        if datos_actualizados:
            usuario_actualizado = usuario_crud.actualizar_usuario(
                db, db_obj=usuario, obj_in=datos_actualizados
            )
...
def cambiar_estado_usuario(db: Session) -> None:
    ...
    try:
        ...
        if confirmar == "s":
            ...
            usuario_actualizado = usuario_crud.actualizar_usuario(
                db, db_obj=usuario, obj_in={"activo": nuevo_estado}
            )
