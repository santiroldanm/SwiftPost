...
class SedeCRUD(CRUDBase[Sede, SedeCreate, SedeUpdate]):
    ...
    def get_by_ciudad(
        self, db: Session, ciudad: str, skip: int = 0, limit: int = 100
    ) -> List[Sede]:
        """Get sedes by ciudad."""
        return (
            db.query(Sede).filter(Sede.ciudad == ciudad).offset(skip).limit(limit).all()
        )
    ...
    def create(self, db: Session, *, obj_in: SedeCreate, creado_por: UUID) -> Sede:
        """Create a new sede."""
        db_obj = Sede(**obj_in.dict(), creado_por=creado_por)
        db.add(db_obj)
        db.commit()
        db.refresh(db_obj)
        return db_obj
