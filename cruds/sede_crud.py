...
class SedeCRUD(CRUDBase[Sede, SedeCreate, SedeUpdate]):
    ...
    def obtener_por_ciudad(
        ...
        try:
            ...
            print(f"Error de base de datos al obtener sedes por ciudad {ciudad}: {str(e)}")
            try:
                db.rollback()
            except:
                pass
    ...
    def crear_registro(
        ...
        *, 
        ...
        """
        try:
            # Verificar el estado de la transacción antes de comenzar
            if db.is_active and db.in_transaction():
                try:
                    db.rollback()
                except:
            ...
            if 'nombre' in datos:
                sede_existente = db.query(Sede).filter(Sede.nombre == datos['nombre']).first()
                if sede_existente:
                    print(f"Error: Ya existe una sede con el nombre '{datos['nombre']}'")
                    return None
        ...
        *,
        ...
        try:
            ...
            if 'nombre' in datos_actualizados:
                sede_existente = db.query(Sede).filter(
                    Sede.nombre == datos_actualizados['nombre'],
                    Sede.id_sede != objeto_db.id_sede
                ).first()
                if sede_existente:
                    print(f"Error: Ya existe otra sede con el nombre '{datos_actualizados['nombre']}'")
                    return None
    ...
    def desactivar(self, db: Session, *, id: UUID, actualizado_por: UUID) -> Optional[Sede]:
        """
        Desactiva una sede.
    ...
    def obtener_por_nombre(self, db: Session, nombre: str) -> Optional[Sede]:
        ...
        try:
            if not nombre or not nombre.strip():
                return None
            return db.query(Sede).filter(Sede.nombre.ilike(f"%{nombre.strip()}%")).first()
        except Exception as e:
