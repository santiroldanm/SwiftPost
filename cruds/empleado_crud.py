...
class EmpleadoCRUD(CRUDBase[Empleado, EmpleadoCreate, EmpleadoUpdate]):
    ...
    def __init__(self):
        ...
        self.cargos_permitidos = ['administrador', 'coordinador', 'mensajero', 'atencion_cliente', 'secretario', 'logistico']
    ...
    def _validar_datos_empleado(self, datos: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Validar segundo_nombre (opcional)
            if 'segundo_nombre' in datos and datos['segundo_nombre'] and not self._validar_longitud_texto('segundo_nombre', datos['segundo_nombre']):
            ...
            # Validar segundo_apellido (opcional)
            if 'segundo_apellido' in datos and datos['segundo_apellido'] and not self._validar_longitud_texto('segundo_apellido', datos['segundo_apellido']):
            ...
            if datos.get('tipo_empleado') not in self.cargos_permitidos:
                errores.append(f"tipo_empleado: '{datos.get('tipo_empleado')}' no es válido. Opciones: {', '.join(self.cargos_permitidos)}")
    ...
    def crear_registro(
        ...
        *,
        ...
        if 'documento' in datos and self.obtener_por_documento(db, datos['documento']):
            print(f" Error: Ya existe un empleado con este número de documento: {datos['documento']}")
            return None
        ...
        if 'correo' in datos and self.obtener_por_email(db, datos['correo']):
            print(f" Error: Ya existe un empleado con este correo electrónico: {datos['correo']}")
            return None
        ...
        try:
            ...
            empleado_existente = db.query(Empleado).filter(Empleado.id_empleado == str(usuario_id)).first()
        ...
        try:
            datos_empleado = {k: v for k, v in datos.items() if k not in ['id_empleado', 'usuario_id', 'creado_por']}
            
            if 'tipo_documento' in datos_empleado and isinstance(datos_empleado['tipo_documento'], str):
                from uuid import UUID
                datos_empleado['tipo_documento'] = UUID(datos_empleado['tipo_documento'])
