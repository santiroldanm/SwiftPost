...
def crear_usuario_administrador(db: Session):
    ...
    # Crear el usuario administrador sin hashing de contraseña
    usuario_admin = Usuario(
        id_rol=rol_admin.id_rol,
        nombre_usuario="admin",
        password="admin123",  # Contraseña en texto plano (el sistema se encargará del hashing)
        activo=True,
...
def crear_tipos_documento_por_defecto(db: Session, id_usuario_creador: str):
    ...
    documentos_por_defecto = [
        ...
        {"nombre": "Cédula de Extranjería", "codigo": "CE", "numero": 3, "activo": True},
    ...
    for datos_documento in documentos_por_defecto:
        # Verificar si ya existe un tipo de documento con el mismo código o nombre
        documento_existente = db.query(TipoDocumento).filter(
        ...
        if not documento_existente:
            ...
            contador_creados += 1
            print(f"Tipo de documento creado: {datos_documento['nombre']} ({datos_documento['codigo']})")
...
def principal():
    ...
    try:
        ...
        # Crear los tipos de documento por defecto
